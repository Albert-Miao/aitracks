from baseline_cnn_0_1_2 import *
...
def train_network():
    ...
    for epoch in range(100):
        ...
        for minibatch_count, row in train_df.iterrows():
            ...
            target = torch.FloatTensor(row[['lat', 'lon']].to_list())[None, :]
            ...
            if minibatch_count in reset_inds:
                h = Variable(torch.zeros(10, 1, 200).requires_grad_())
                c = Variable(torch.zeros(10, 1, 200).requires_grad_())
                h, c = h.to(computing_device), c.to(computing_device)
                output, (h, c) = net(image, bb_coords=bb_coords, prev_h=h, prev_c=c)

            else:
                h, c = Variable(h.data), Variable(c.data)
                h, c = h.to(computing_device), c.to(computing_device)
                output, (h, c) = net(image, prev_h=h, prev_c=c)
        ...
        with torch.no_grad():
            for minibatch_count, row in val_df.iterrows():
                ...
                if minibatch_count in reset_inds:
                    h = Variable(torch.zeros(10, 1, 200))
                    c = Variable(torch.zeros(10, 1, 200))
                    h, c = h.to(computing_device), c.to(computing_device)
                    output, (h, c) = net(image, bb_coords=bb_coords, prev_h=h, prev_c=c)
