...
class Nnet(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.RNN(200, 200, 10, nonlinearity='relu')
    ...
    def forward(self, input, bb_coords=None, prev_h=None):
        x = self.main(input)
        ...
        if not prev_h is None:
            x, h = self.rnn(x.view(1, 1, -1), prev_h)
        else:
            x, h = self.rnn(x.view(1, 1, -1))

        return self.fc2(x.view(1, -1)), h
