...
class Nnet(nn.Module):
    ...
    def forward(self, input, bb_coords=None, prev_h=None, prev_c=None):
        x = self.main(input)
        x = self.fc1(x.view(1, -1))

        if not bb_coords is None:
            x = torch.hstack((x, bb_coords))
            x = self.reset(x)

        if not prev_h is None:
            x, (h, c) = self.rnn(x.view(1, 1, -1), (prev_h, prev_c))
        else:
            x, (h, c) = self.rnn(x.view(1, 1, -1))

        return self.fc2(x.view(1, -1)), (h, c)
